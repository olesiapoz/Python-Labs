...
def squarer(a, b):
    ...
    return make_dual_constraint(a, b, mul, sqrt)
...
def make_pythagorean(a, b, c):
    ...
    d, e, f = [make_connector() for _ in range(3)]
    squarer(a,d)
    squarer(b,e)
    squarer(c,f)
    adder(d,e,f)
...
def make_dual_constraint(a, b, aa, bb):
    """The constraint that aa(a,a)=b and bb(b)=a and cb(c,b)=a."""
    def new_value():
        av, bv = [connector['has_val']() for connector in (a, b)]
        if av:
            b['set_val'](constraint, aa(a['val'], a['val']))
        elif bv:
            a['set_val'](constraint, bb(b['val']))
    def forget_value():
