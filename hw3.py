...
def make_center_percent(c, p):
    ...
    >>> str_interval(make_center_percent(-10, 50))
    '-15.0 to -5.0'
    """
    "*** YOUR CODE HERE ***"
...
def sum_nonzero_with_map_filter_reduce(seq):
    ...
    Map applies a function to each element in a sequence and collects the results.
    The filter function takes a sequence and returns those elements of a sequence
    ...
    """
    def filtered(seq):
        seq1 = filter(non_zero, seq)
        return seq1

    mapped = map(square_interval,(filtered(seq)))
...
def sum_nonzero_with_generator_reduce(seq):
    ...
    return reduce(add_interval,(square_interval(q) for q in seq if non_zero(q)==True))
