...
def make_withdraw(balance, password):
    ...
    def make_withdraw(balance, password):
        ...
        """
        empty_list = None
        p = [empty_list, empty_list, empty_list]
        i = 0
        ...
        def protected_withdraw(amount, psw):
            nonlocal password
            nonlocal p
            nonlocal i
            if psw == password and p[2] == empty_list:
                return withdraw(amount)
            elif psw != password and p[2] == empty_list:
                p[i] = psw
                i += 1
                return 'Incorrect password'
            return "Your account is locked. Attempts: {0}".format(p)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def joint(amount, psw):
        ...
        nonlocal withdraw
        if psw == old_password or psw == new_password:
